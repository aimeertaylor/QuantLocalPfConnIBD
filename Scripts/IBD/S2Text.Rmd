...
Empirical density plots (Fig \ref{fig:histMLA} to \ref{fig:histMLA_MKT}) suggest that $\hat{\pi}_{\text{IBD}}$ for pairs of parasite samples within and across clinics loosely follow a mixture distribution over three classes: 1) near zero $\hat{\pi}_{\text{IBD}}$ shown in green; 2) low to intermediate $\hat{\pi}_{\text{IBD}}$ shown in shades of blue; and 3) high $\hat{\pi}_{\text{IBD}}$, which notably rarefies with distance, and is colored red if sufficiently dense to be visible, and otherwise outlined in black. 

We interpreted near zero $\hat{\pi}_{\text{IBD}}$ as comparisons between parasites whose relatives were either not sampled or where barcode SNPs were to few to resolve IBD; low to intermediate $\hat{\pi}_{\text{IBD}}$ as comparisons between panmictically mixing parasites; and high $\hat{\pi}_{\text{IBD}}$ as comparisons including recent parasite migrants between clinics.

To distinguish recent migrants, we selected a threshold equal to 0.5 (dotted vertical lines, Fig \ref{fig:histMLA} to \ref{fig:histMLA_MKT}), classing all comparisons above the threshold highly related. We then constructed logistic regression models, as outlined below, to quantify distance as a correlate of the probability of a comparison being classed highly related.
...
#===========================================================
par(mfrow = c(1,2), pty = 's', family = "serif")
...
legend('right', pt.bg = 1:4, pch = 21, legend = names(Time_models)[-5], bty = 'n', cex = 0.7)
...
```{r, include = TRUE, fig.cap=paste("\\label{fig: coeff with model}Spatial trend estimates, $\\hat{\\beta}_1$ under models 1 to 10 with time measured in weeks.")}
#====================================================================================
...
plot(y = beta1s, x = 1:10, ylim = c(-0.1, 0.1), bty = 'n', xaxt = 'n',
     ...
     ylab = expression(beta[1] ~ 'estimate'))
axis(side = 1, at = 1:10)
...
where $\text{Year 2014}_i$ was TRUE if both parasite samples were collected in 2014 and false otherwise, thereby accounting for a large increase in $\hat{\pi}_{\text{IBD}}$ in 2014 (Fig \ref{fig: 2014 IBD}), and resulting in a decrease in AIC of $`r reduction`$ compared with model 8. 

```{r, include = TRUE, fig.cap=paste("\\label{fig: 2014 IBD}Fraction of highly related parasite sample pairs (pairs with $\\hat{\\pi}_{\\text{IBD}}$ > 0.5) over time")}
#===============================================================================
...
Thus far we have considered spatial trends in the $\mathbb{P}(\hat{\pi}_{\text{IBD}_i} > 0.5)$ (see Tables 2 and 3 of the main manuscript for regression coefficient estimates) because empirical density plots suggested $\hat{\pi}_{\text{IBD}}$ were loosely distributed according to different classes, with perceived recent migrants having high $\hat{\pi}_{\text{IBD}}$ (Fig \ref{fig:histMLA} to S2.\ref{fig:histMLA_MKT}). Alternatively, we could regress $\hat{\pi}_{\text{IBD}}$ directly onto distance by letting $z_i = \hat{\pi}_{\text{IBD}_i}$. Under chosen models 2 and 8 (and 11 for WGS data) with $z_i = \hat{\pi}_{\text{IBD}_i}$, the spatial trend estimates were mostly negative, though very small, and those that are significant in Tables 2 and 3 of the main manuscript are similarly significant here (Table \ref{tab: direct reg}). 
...
for(i in 1:3){
  X <- as.numeric(rownames(signifneg_results[-1,])) # Remove 25 as too few 
...
```{r Attrition_plot_unconstrained}
#========================================================================
...
# Remove 25 since too few points and add some plots to anchor: 
null <- 0.025; # Because two-sided p-values
anchorlength <- 2
signifneg_results_anchored = rbind(matrix(null, ncol = 3, nrow = anchorlength), signifneg_results[-1, ]) 
rownames(signifneg_results_anchored)[1:anchorlength] <- rep('0', anchorlength)

par(mfrow = c(1,1), mar = c(5,5,1,1), pty = 's', family = "serif")
cols <- c('gray', 'blue', 'black')
max_sample_size <- 1200
plot(NULL, xlim = c(0,max_sample_size), ylim = c(0,1), bty = 'n', panel.first = grid(), 
     xlab = 'Subset size (samples)',
     ylab = expression('Proportion of significant negative' ~ beta ~ 'values'), cex = 0.75)
legend(y = 0.3, x = 400, legend = c('WGS (34911 SNPs)', '93-SNP barcode', '24-SNP barcode'), pt.bg = 1:3,
...
for(i in 1:3){
  X <- as.numeric(rownames(signifneg_results_anchored[,]))
  Y <- signifneg_results_anchored[,i]
  b <- exp(1)
  a <- -log((null/(1-null)), base = b)
