# Modifications by Aimee Taylor, Aug17 inc. addition of myplotFinestructure
# and alteration of fs.plot.dendrogram to colour leaves by site, 
# which is currently done in a hacky way using name, 
# (would ideally add a colour to subtreet attribute). 
##################################################################
...
fs.plot.dendrogram <- function(x, type = c("rectangle", "triangle"), center = FALSE,
           ...
           nodePar, edgePar, horiz = FALSE)
  {
    ...
    if (is.leaf(subtree)) {
      ...
      if (leaflab == "perpendicular") { # somewhat like plot.hclust
        ...
        text(X, Y, 'o', xpd = TRUE, srt = srt, adj = adj,
             cex = lab.cex, col = which(sites == nodeText), font = lab.font, crt=lab.crt) # Hacky addition by Aimee to colour leaves
      }
    }
    else if (inner) {
...
myplotFinestructure<-function(tmpmat,# this is the heatmap to be plotted
                              labelsx,
                              labelsy=NULL,# assumed equal to labelsx unless otherwise specified
                              labelsatx=NULL, # location of the X labels - note that the heatmap X ranges 1:dim(tmpmat)[1] and good locations are (1:dim(tmpmat)[1]) -0.5
                              labelsaty=NULL, # assumed equal to labelsatx unless otherwise specified
                              cols=NULL,# colour scale. Will be generated by MakeColors if NULL
                              colticks = NULL, 
                              regression=NULL, # optional regression to be placed at the top
                              regressionX=NULL,
                              regressionY=NULL, 
                              optpts=NULL, # optional points to add, e.g. MAP state
                              labmargin=8, # margin for x,y labels
                              layoutd=0.2, # proportion of the plot to be used by the regression
                              layoutf=0.1,# proportion of the plot to be used by the scale
                              cex.axis=0.5, # cex.axis applies to the x,y labels
                              crt=0, # character rotation of x,y labels (x is rotated the opposite direction)
                              colscale=NULL, # specify a colour scale, meaning the range of the z-values for the image
                              ...
                              ignorebelow=0, # if text.col=0, then we white out everything below ignorebelow
                              ignoreabove=Inf, # Ignore values above this
                              regressionmar=c(8,2,2,1), # additional modification to the margins of the regression
                              scalemar=c(2,5,2,1), # additional modification to the margins of the scale
                              hmmar=c(0,0,5,1), # additional modification to the margins of the heatmap
                              cex.scale=1, # size of the characters in the scale
                              scalelocs=NULL, # optional forced positions of the scale
                              scalenum=10, # if scalelocs=NULL, the number of points to label on the scale
                              scalesignif=3, # number of significant digits on scale
                              scalelabel="", # label for the scale
                              optcex=1.0, # size of the optional points
                              ...
                              optcol="black", # colour for the optional points
                              labelsoff=c(1,1), # "margin" like, distance from colour/rotated labels and the axis
                              tickmarks=0,# size of tickmarks when using colour/rotated labels
                              startpt="bottomleft", # whether the first individual vs first individual is at the "topleft", or the "bottomleft"
                              dolayout=TRUE,
                              main="", # regression title
                              cex.main=1, # title size
                              adj=0 # title adj)
) {
  if(dolayout) layout(matrix(c(1,1,3,2), 2, 2,  byrow=TRUE),
                      widths=c(1-layoutf,layoutf),
                      height=c(layoutd, 1-layoutd))
  if(is.null(cols)) cols<-MakeColorYRP()
  ## TOP RIGHT 
  if(dolayout || !is.null(regression))
  {
    ## TOP LEFT: REGRESSION
    par(mar=c(0,labmargin,0,0)+regressionmar, pty = 's')
  }
  if(dolayout && is.null(regression)){
    plot(c(0,1),c(0,1),type="n",axes=F,xlab="",ylab="",main=main,cex.main=cex.main,adj=adj)# null plot for top right
  }else if(!is.null(regression)) {
    
    ## Aimee: add regression plot here (ie dend == regression)
    plot(y = regressionY, 
         x = regressionX, 
         xlim = c(0,100), 
         xlab = 'Distance (km)',
  ...
  ## BOTTOM LEFT: MAIN HEATMAP
  par(mar=c(labmargin,labmargin,0,0)+hmmar, pty = 's')
  if(is.null(labelsatx)) { # labels x positions
    labelsatx<-rep('', dim(tmpmat)[2]) 
  }
  if(is.null(labelsaty)) { # labels y positions
    labelsaty<- labelsatx
  }
  ...
  #plot the heatmap
  image(1:dim(tmpmat)[2],1:dim(tmpmat)[1],t(tmpmat),xaxt="n",yaxt="n",xlab="Donor",ylab="Recipient",col=cols,zlim=colscale)
  # draw the axes
  if(crt==0 && is.null(text.col)) {
    axis(3,at=labelsatx,labels=labelsx,las=2,cex.axis=cex.axis)
    axis(2,at=labelsaty,labels=labelsy,las=2,cex.axis=cex.axis)
  }else{
    text(labelsatx, labelsoff[1]+dim(tmpmat)[2]+4, srt = 90-crt, adj = 1,
         labels = labelsx, xpd = TRUE,cex=cex.axis,col=text.col)
    text(-labelsoff[2], labelsaty, srt = crt, adj = 1,
         labels = labelsy, xpd = TRUE,cex=cex.axis,col=rev(text.col))
    if(tickmarks>0) {
      axis(side=3,at=labelsatx,labels=rep("",length(labelsatx)),las=2,cex.axis=cex.axis)
      axis(side=2,at=labelsaty,labels=rep("",length(labelsaty)),las=2,cex.axis=cex.axis)
    }
