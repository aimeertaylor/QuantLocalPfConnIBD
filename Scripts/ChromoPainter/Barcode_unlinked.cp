...
############## Command history:
section:history
CMDLINE: -phasefiles ./Barcode.phase -idfile ./Barcode.ids -ploidy 1 -dos2 
CMD:-phasefiles ./Barcode.phase
CMD:-idfile ./Barcode.ids
CMD:-ploidy 1
CMD:-countdata
CMD:-makes2
CMD:-dos2
...
idfile:./Barcode.ids  # IDfile location, containing the labels of each individual. REQUIRED, no default (unless -createids is used).
s12args:  # arguments to be passed to Chromopainter (default: empty)
## Quantities observed from data

ninds:1173  # Derived. number of individuals observed in the idfile
nindsUsed:1173  # Derived. number of individuals retained for processing from the idfile
nsnps:93  # Derived. number of SNPs in total, over all files
...
s2outputroot:./Barcode_unlinked_tmp_mainrun.unlinked_haploid  # Output file name for stage 2 (default: autoconstructed).
...
phasefiles:./Barcode.phase  # Comma or space separated list of all 'phase' files containing the (phased) SNP details for each haplotype. Required. Must be sorted alphanumerically to ensure chromosomes are correctly ordered. So don't use *.phase, use file{1..22}.phase. Override this with upper case -PHASEFILES.
...
nsnpsvec:93  # Derived. Comma separated list of the number of SNPs in each phase file.
...
s2outputrootvec:./Barcode_unlinked/stage2/./Barcode_unlinked_tmp_mainrun.unlinked_haploid_file1_ind1-1173  # Derived. Comma separated list of the stage 2 output files names.
