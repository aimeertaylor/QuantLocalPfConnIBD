...
\makeatletter
\renewcommand{\fnum@figure}{Fig S4.\thefigure}
\makeatother
...
```{r, include = TRUE, fig.cap=paste("\\label{fig: barcode}93 and 24-SNP barcode positions"), fig.width = 5, fig.height = 5}
#===============================================================================================
...
nrow(WGSf24) 
```
```{r, include = TRUE, fig.cap=paste("\\label{fig: WGS 93 24}Comparison between $\\hat{\\pi}_{\\text{IBD}}$ based on the whole genome and 93 and 24 SNP subsets generated using 2001-2014 WGS data."), fig.width = 5, fig.height = 5}
#===============================================================================
...
In other words, when distances are sufficiently large that $e^{-k\rho d_t} \approx 0$, SNPs are effectively independent of one another. Fig S4.\ref{fig: switch} shows that for $\rho = 7.4\times 10^{-7}$ base pairs per Morgan [@Miles2016], and small numbers of generations, $k < 50$, $e^{-k\rho d_t} \neq 0$ for most distances between barcode SNPs. That is to say, barcode SNPs are dependent under hmmIBD providing $k$ is small because $\rho$ is low. 
...
```{r, include = TRUE, fig.cap=paste("\\label{fig: zeroth}IBD proportion estimates generated hmmIBD allowing dependence and forcing independence between SNPs by varying $\\rho$."), fig.width = 5, fig.height = 5}
# =============================================================
